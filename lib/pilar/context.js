...
export async function gatherPilarContext(question) {
  ...
  const [farmaciasHoy, farmaciasSemana, promociones, eventos, noticias, negocios] = await Promise.all([
  ...
  return {
    ...
    zonaDetectada: zona,
    intent,
    farmaciasHoy: farmaciasHoy.map(({ nombre, direccion, localidad, telefono, horario, notas }) => ({
      nombre,
      direccion,
      localidad,
      telefono,
