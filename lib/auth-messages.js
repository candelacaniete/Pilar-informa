...
export function resetErrorMessage(resetError, redirectTo) {
  ...
  if (
    ...
    code.includes('pgrst125')
  ) {
    return 'La URL de Supabase en Vercel está mal. NEXT_PUBLIC_SUPABASE_URL debe ser solo https://TU-PROYECTO.supabase.co (Supabase → Settings → API → Project URL). Sin /rest/v1, sin barra final, y no la URL de la web. Después redeployá.'
  }
  ...
  if (
    ...
    return `Supabase no acepta la URL de retorno. En Authentication → URL Configuration agregá esta Redirect URL: ${redirectTo || 'https://guia-pilar.com/**'}`
  }
