...
const TECH_ALLOWLIST_SLUGS = ['katem', 'konstruct']

function mockTechAllowlist() {
  return mockNegocios.filter(
    (n) => n.estado === 'activo' && TECH_ALLOWLIST_SLUGS.includes(n.slug),
  )
}

/** Si Tecnología está vacía o faltan Katem/Konstruct, los completa desde mock. */
function ensureTechListings(list = [], categoriaSlug = null) {
  if (categoriaSlug && categoriaSlug !== 'tecnologia') return list

  const bySlug = new Map(list.map((n) => [n.slug, n]))
  for (const demo of mockTechAllowlist()) {
    if (!bySlug.has(demo.slug)) bySlug.set(demo.slug, demo)
  }

  // En listado general también deben aparecer.
  if (!categoriaSlug) {
    return sortNegocios([...bySlug.values()])
  }

  return sortNegocios(
    [...bySlug.values()].filter((n) => n.categorias?.slug === 'tecnologia' || TECH_ALLOWLIST_SLUGS.includes(n.slug)),
  )
}

function ensureTechCategoria(cats = []) {
  const list = [...cats]
  const idx = list.findIndex((c) => c.slug === 'tecnologia')
  if (idx >= 0) {
    list[idx] = { ...list[idx], cerrada: true, nombre: list[idx].nombre || 'Tecnología' }
    return list
  }
  const fromMock = mockCategorias.find((c) => c.slug === 'tecnologia')
  if (fromMock) list.push({ ...fromMock, cerrada: true })
  return list.sort((a, b) => (a.orden ?? 99) - (b.orden ?? 99))
}

// No pedir `cerrada` en el embed: si la migración 003 aún no corrió en Supabase,
// el select falla y la guía queda vacía. `cerrada` se resuelve vía getCategorias().
const negocioSelect = `
  *,
...
export async function getCategorias() {
  ...
  if (!supabase) return ensureTechCategoria(mockCategorias)
  const { data, error } = await supabase.from('categorias').select('*').order('orden')
  if (error || !data?.length) return ensureTechCategoria(mockCategorias)
  return ensureTechCategoria(data)
}
...
export async function getNegociosActivos({ categoriaSlug, destacados, limit } = {}) {
  ...
  if (!supabase) {
    ...
    list = sortNegocios(ensureTechListings(list, categoriaSlug))
  ...
  let list = sortNegocios(ensureTechListings(data, categoriaSlug))
  if (destacados) {
    list = list.filter((n) => n.plan === 'premium' || n.plan === 'destacado')
  }
  return limit ? list.slice(0, limit) : list
...
export async function getNegocioBySlug(slug) {
  ...
  if (data) return data
  // Fallback: fichas allowlist Tecnología (Katem / Konstruct) si aún no están en Supabase.
  if (TECH_ALLOWLIST_SLUGS.includes(slug)) {
    return mockNegocios.find((n) => n.slug === slug && n.estado === 'activo') || null
  }
  return null
}
