...
export async function getAdminMetrics() {
  ...
  const [
    ...
    supabase
      .from('negocios')
      .select('id, nombre, slug, categoria_id, plan, estado, plan_vence, fuente_alta, creado_en'),
    ...
    supabase.rpc('pilar_stats_mes', { p_mes: monthStart }),
  ])
  ...
  const proyeccion = buildProyeccion({
    ...
    proyectadoRow: proyeccionMesRes.error ? null : proyeccionMesRes.data,
  })

  const renovacion = buildRenovacion(eventosRes.error ? [] : eventosRes.data || [])

  const pilarRow = !pilarRes.error && Array.isArray(pilarRes.data) ? pilarRes.data[0] : null
  const pilar = {
    consultas: Number(pilarRow?.consultas || 0),
    usuariosUnicos: Number(pilarRow?.usuarios_unicos || 0),
  }

  const metas = buildMetasProgress(metasRes.error ? [] : metasRes.data || [], negociosCards.activos)
  ...
  return assembleMetrics({
    ...
    metas,
    fuentesAlta,
    proyeccionesList: proyeccionesListRes.error ? [] : proyeccionesListRes.data || [],
    metasList: metasAllRes.error ? [] : metasAllRes.data || [],
  })
}
